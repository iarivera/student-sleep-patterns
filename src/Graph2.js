...
function Graph2({ data }) {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .selectAll('rect')
      .data(d => genderGroups.map(gender => ({ key: gender, value: d[gender], year: d.University_Year }))) // Map each gender to a value
      .join('rect')
      .attr('x', d => x1(d.key)) // Position each gender bar side by side
      .attr('y', d => y(d.value)) // Y position depends on the value (height)
      .attr('height', d => y(0) - y(d.value)) // Height of each segment based on the value
      .attr('width', x1.bandwidth()) // Width of each bar for a gender
      .attr('fill', d => color(d.key)) // Color each segment by gender
      .on('mouseover', (event, d) => {
        tooltip.style('opacity', 1);
        tooltip.html(
          `Gender: ${d.key}<br>Average Sleep Duration: ${d.value} hrs<br>University Year: ${d.year}`
        );
      })
      .on('mousemove', event => {
        tooltip
